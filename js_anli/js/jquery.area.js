...
function selectC(p,c) {
	...
	if (expressArea == province[p] + '-' +'市辖区' || expressArea == province[p] + '-' +'市辖县') {
		...
		//城市选择
		$('#lvscity').val(province[p]);
	}else{
		$("#expressArea dl dd").html(expressArea);
		//城市选择
		$('#lvscity').val(expressArea);
	}
...
function clockArea() {
	...
	$('#remen').show();
	//锚点定位
	console.log(express_text)
	if (express_text != '') {
		$('html,body').animate({
			scrollTop:$('#column1 ul .lis:last-child').offset().top-80+"px" 
		}, 500);
	}
	intProvince();
	var express_text = $('#expressArea dl dd').html();
